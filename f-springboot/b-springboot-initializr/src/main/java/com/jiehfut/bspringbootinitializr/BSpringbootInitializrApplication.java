...
public class BSpringbootInitializrApplication {
     ...
     * springboot 启动会默认加载 142 个配置类，在 META-INF/spring/org.springframework.boot.autoconfigure.imports 类中
     * 这个注解的作用就是在项目启动的时候，利用 @Import(***.class) 注解
     * 批量导入机制把 spring-boot-autoconfigure 包下的142个***AutoConfigration 自动配置类导入进来（自动配置）
     ...
     *        用来把配置文件中的指定前缀的属性封装 b  b到 ***Properties 属性类中，如此是服务器配置都是以 server 开头的配置，进行属性绑定
